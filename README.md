...
| `child_female` | 目标 250 Hz（`--ref` 190） | ×1.18（固定） | +0.50 dB/oct | – | – | **女声**→儿童音，步长更小 |
...
你的录音是**女声（F0 ≈ 250 Hz）**时：`child` 会把音高推到约 350 Hz，偏尖；
...
| child | ×1.400 | **×1.400** | ×1.150（固定） | ×1.149 |
| child_bright | ×1.567 | **×1.561** | ×1.220（固定） | ×1.221 |
| child_female | ×1.316 | **×1.351** | ×1.180（固定） | ×1.220 |
...
**儿童音预设的"成对"检查**：`child`(210/150, ×1.15)、`child_bright`(235/150, ×1.22)、
`child_female`(250/190, ×1.18) 三组都必须同时满足"倍率命中各自设计点"且"共振峰倍率 **小于**
音高倍率"。后者是真正的物理约束（儿童声道长度比 1.14–1.25 < 基频比 1.5–2.0），
只改其中一个数字会立刻被这条抓住。
...
| `AB_05_F_child_prev.wav` | 修复前的 `child`（旧抗混叠） |
