...
| `child_female` | 目标 250 Hz（`--ref` 190） | ×1.18（固定） | +0.50 dB/oct | – | – | **女声**→儿童音（实为"女声→女声"，见下） |
...
> **250 Hz**，`child_bright` 232.7 → **235 Hz**。女声录音本来就命中目标，不受影响。
>
...
你的录音是**女声（F0 ≈ 250 Hz）**时：`--ref auto` 下 `child` 会把音高**降到**约 210 Hz
（实测倍率 ×0.84）——这正是女声→童声该做的，因为成年女声与儿童基频范围本就重叠，
差别主要在声道；想更接近幼童可用 `child_female`（250 Hz，倍率 ×1.00，几乎不动音高），
或显式控制：`--target 300 --formant 1.18`。
...
**儿童音预设的"成对 + 上限 + 顺序"检查**：`child`(210/150, ×1.15)、`child_bright`(235/150, ×1.22)、
`child_female`(250/190, ×1.18) 三组都必须同时满足四件事：
...
④**在 130 Hz 的男声区间里三个预设的输出必须随目标递增**（210 < 235 < 250 Hz）。
第 ④ 条是这个 bug 的直接回归测试：上限写成设计倍率时顺序会变成
`child 1.400 > child_bright 1.567 > child_female 1.316`，也就是"目标最低的那个最尖"。
