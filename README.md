...
|---|---|---|---|---|---|---|
| `child` | 目标 235 Hz（按 `--ref`，默认 150 Hz） | ×1.22（固定） | +1.00 dB/oct | – | – | **男声**→儿童音 |
| `child_soft` | 目标 210 Hz | ×1.15（固定） | +0.50 dB/oct | – | – | 同上但更柔、更不刺耳 |
...
> **共振峰因子默认是"固定值"**：`child` 永远是 ×1.22、`child_female` 永远是 ×1.18，
> 不随 `--ref`/自动参考算出的音高倍率变动（`--formant-track` 才开启按音高缩放，
> 属实验选项，默认关闭）。理由与实测见 §6 第 7 条。
>
> **`child` / `child_female` / `child_soft` 预设内置 `--nfft 1024 --hop 256`**（其余预设
...
| child | ×1.567 | **×1.561** | ×1.220（固定） | ×1.221 |
...
会生效的区间）下必须给出 ×1.220 且 `formant_tracked = false`；`--formant-track` 必须给出
...
| 量 | 数值 | 说明 |
|---|---|---|
| F0 | 148.9 → 235 Hz（×1.567） | 儿童（约 5–8 岁）实测 F0 通常 250–300 Hz |
| 共振峰 | ×1.22（×1.57 变调后由掩模补回 ×0.779） | 儿童声道约短 20–25 % |
| 能量谱质心 | 733 → ~1350 Hz | 变亮是必然的——变调把**所有**频率一起乘 1.567 |

关键点：儿童与成年男性的声道长度比（约 1.14–1.25）**小于**基频比（约 1.5–2.0），
所以正确做法是共振峰放大**少于**音高。当前 ×1.22 落在真实儿童区间内；把共振峰调到跟音高
一样大（×1.57）才会变成卡通尖声。因此**想让"音调不那么高"必须把 `--target` 与 `--formant`
一起降**——只降音高会让声道听起来比音高暗示的更小，反而更尖。`child_soft` 就是按这条做的
（目标 210 Hz + 共振峰 ×1.15），实测质心 ~1356 → ~1244 Hz。

**③ "机械感"的根因是相位声码器本身，只能换算法**：
...
已做完的缓解：抗混叠 + 儿童音预设改 1024/256 长窗。**要根治需要换基音同步算法
（TD-PSOLA：按基音周期对齐地复制/删除整周期，不做相位递推）**——这是语音变调里标准的
"无金属感"做法，代价是要新写周期标记检测与 PSOLA 合成，属于一次较大的改动，
**目前没有实现**。
...
| `AB_01_B_child_now.wav` | 当前 `child`（抗混叠 + 1024/256） |
| `AB_02_C_child_soft.wav` | `child_soft`（F0 210 Hz，共振峰 ×1.15） |
| `AB_03_D_child_light.wav` | `child` + `--formant 1.12`（只降共振峰） |
